...
class R2GenGPT(pl.LightningModule):
    """
    R2GenGPT model with ChatGLM3 support.
    """
    def __init__(self, args):
        ...
        print('Loading StableLM-3B-4E1T')
        ...
        if args.llm_use_lora:
            ...
            self.lm_model = get_peft_model(self.lm_model, peft_config)
            self.lm_model.print_trainable_parameters()
            print('Loading StableLM LoRA Done')         
        else:
            self.embed_tokens = self.lm_model.get_input_embeddings()
            for name, param in self.lm_model.named_parameters():
                param.requires_grad = False
            print('Loading StableLM Done')

        self.lm_proj = nn.Linear(self.visual_encoder.num_features, self.lm_model.config.hidden_size)
        self.layer_norm = nn.LayerNorm(self.lm_model.config.hidden_size)
    ...
    def score(self, ref, hypo):
        ...
        scorers = [
            ...
            (Rouge(), "ROUGE_L"),
            # (Meteor(), "METEOR"), # METEOR is not process-safe for DDP, causing BrokenPipeError.
            (Cider(), "CIDEr")
        ]
        final_scores = {}
    ...
    def encode_img(self, images):
        ...
        inputs_lm = self.lm_proj(image_embeds)
        atts_lm = torch.ones(inputs_lm.size()[:-1], dtype=torch.long).to(image.device)
        return inputs_lm, atts_lm
    ...
    def validation_step(self, samples, batch_idx):
        ...
        bos = torch.ones([batch_size, 1],
                         ...
                         device=atts_img.device) * self.lm_tokenizer.bos_token_id
        bos_embeds = self.embed_tokens(bos)
        atts_bos = atts_img[:, :1]
